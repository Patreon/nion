...
const SUPPORTS_CAPTURE_STACK_TRACE = typeof Error.captureStackTrace === 'function';

function coerceDeclaration(declaration) {
  return typeof declaration === 'string' ? { dataKey: declaration } : declaration;
}
...
function useNion(declaration, deps = EMPTY_DEPS) {
  ...
  const deleteRes = useCallback(
    (params, actionOptions) => {
      ...
      opt.refToDelete = actionOptions?.refToDelete ? actionOptions?.refToDelete : { id: objId, type: objType };
