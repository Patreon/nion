...
describe('nion: reducers', () => {
  describe('entities reducer', () => {
    ...
    it('should handle basic NION_API_SUCCESS instantiation', () => {
      ...
      const action = makeActionWithFragment(types.NION_API_SUCCESS, storeFragment.toPayload());
    ...
    it('should handle basic NION_API_BOOTSTRAP instantiation', () => {
      ...
      const action = makeActionWithFragment(types.NION_API_BOOTSTRAP, storeFragment.toPayload());
      reducer.applyAction(action);
      const name = get(reducer.state, ['user', 123, 'attributes', 'name']);
      expect(name).toEqual('Test User');
    });
    ...
    it('should handle NION_API_SUCCESS / NION_API_BOOTSTRAP relationships', () => {
      ...
      storeFragment.addEntity('user', 123, { name: 'Test User' }, { campaign: 456 });
      storeFragment.addEntity('campaign', 456, { name: 'Test Campaign' }, { user: 123 });

      const action = makeActionWithFragment(types.NION_API_SUCCESS, storeFragment.toPayload());
      reducer.applyAction(action);
      const campaignId = get(reducer.state, ['user', 123, 'relationships', 'campaign']);
    ...
    it('subsequent SUCCESS / BOOTSTRAP actions should add additional attributes', () => {
      ...
      const firstAction = makeActionWithFragment(types.NION_API_SUCCESS, storeFragment.toPayload());
      ...
      const secondAction = makeActionWithFragment(types.NION_API_SUCCESS, storeFragment.toPayload(true));

      reducer.applyAction(secondAction);
      name = get(reducer.state, `user.${testUserId}.attributes.name`);
      expect(name).toEqual(testName);
      let age = (name = get(reducer.state, `user.${testUserId}.attributes.age`));
    ...
    it('subsequent SUCCESS / BOOTSTRAP actions should add additional relationships', () => {
      ...
      reducer.applyAction(firstAction);
      let campaignId = get(reducer.state, `user.${testUserId}.relationships.campaign`);
      ...
      const secondAction = makeActionWithFragment(types.NION_API_SUCCESS, storeFragment.toPayload());

      reducer.applyAction(secondAction);
      campaignId = get(reducer.state, `user.${testUserId}.relationships.campaign`);
      expect(campaignId).toEqual(firstRelationship.id);

      const bestFriendId = get(reducer.state, `user.${testUserId}.relationships.bestFriend`);
    ...
    it('deletes refs with meta.refToDelete', () => {
      ...
      const action = makeActionWithFragment(types.NION_API_SUCCESS, firstStoreFragment.toPayload());
      reducer.applyAction(action);
      const campaignName = get(reducer.state, `campaign.${456}.attributes.name`);
    ...
    it('deletes relationship refs with meta.refToDelete', () => {
      ...
      expect(get(reducer.state, `user.${123}.relationships.campaigns.data`)).toHaveLength(2);
      ...
      expect(get(reducer.state, `user.${123}.relationships.campaigns.data`)).toHaveLength(1);
      ...
      expect(get(reducer.state, `user.${123}.relationships.campaigns.data`)).toHaveLength(0);
    });
    ...
    it('handles the UPDATE_ENTITY action', () => {
      ...
      const action = makeActionWithFragment(types.NION_API_SUCCESS, firstStoreFragment.toPayload());
      reducer.applyAction(action);
      let campaignName = get(reducer.state, `campaign.${456}.attributes.name`);
      expect(campaignName).toEqual('Test Campaign');
      ...
      const campaignTime = get(reducer.state, `campaign.${456}.attributes.time`);
