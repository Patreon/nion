...
const refsReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case NION_API_SUCCESS:
      if (action.meta.appendKey && action.payload.requestType !== 'jsonApi') {
        const appendKey = action.meta.appendKey;
        const previousEntities = get(state[action.meta.dataKey], appendKey);
        return state.merge(
          {
            [action.meta.dataKey]: {
              [appendKey]: previousEntities.concat(action.payload.responseData.entryRef),
            },
          },
          { deep: true },
        );
